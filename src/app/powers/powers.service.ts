...
export class PowersService {
  ...
  getAll(): Observable<Powers[]> {
    return this.http.get<Powers[]>(this.powersUrl)
      .pipe(
        tap(_ => this.log('fetched Powers')),
        catchError(this.handleError<Powers[]>('getPowers', []))
      );
  }

  getById(id: number): Observable<Powers> {
    return this.http.get<Powers>(`${this.powersUrl}/${id}`)
      .pipe(
        tap(_ => this.log(`fetched power id=${id}`)),
        catchError(this.handleError<Powers>(`getById id=${id}`))
      );
  }
}
